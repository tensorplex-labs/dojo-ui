        ...
        <div className=" border-black">
          ...
          {isLoadingSubmit ? (
            <div className="mt-3 flex items-center justify-center">
              <p><span className='inline-flex animate-spin text-center'><IconLoader /></span> Processing. Please wait ...</p>
            </div>
          ) : (
              ...
              {!showSucceedScreen ? (
                <>
                  <div className="flex flex-col border-b-2 border-black bg-[#F8F8F8] px-5 py-2">
                    <h1 className={`${FontSpaceMono.className} text-xl font-bold tracking-wide text-font-primary`}>MINER APPLICATION </h1>
                  ...
                  <div className={`flex flex-col  border-b-2 border-black px-5 py-2 pb-[41px] ${!getValues('hotkey') && "bg-[#00B6A6]"} bg-opacity-10`}>
                    <div className={`mb-4 mt-[20px] text-base font-bold opacity-80`}><span className={`rounded-3xl border-2 border-black bg-[#00B6A6] px-2 py-1 text-sm uppercase text-white opacity-100 shadow-brut-sm`}>Step 1</span> CONNECT YOUR BITTENSOR WALLET</div>
                    ...
                    <div className=' w-full cursor-pointer p-2' onClick={() => setIsOpen(!isOpen)}>
                      ...
                      {signedInWith && !!jwtToken ? (
                        ...
                        <>
                          <div className='flex flex-col border-2 border-black bg-[#F8F8F8] px-2 py-1 shadow-brut-sm'>
                            <div className='flex items-center justify-between'>
                              <div className='flex items-center py-2'>
                                ...
                                <span className={`${FontManrope.className} text-xl font-bold`}>{selectedAccount?.meta.name}</span>
                              </div>
                              <IconChevronDown
                          ...
                          <div className='mt-4 flex items-center'>
                            ...
                            <span className={`${FontManrope.className} ml-4 text-sm font-medium opacity-50`}>{getFirstFourLastFour(selectedAccount?.address ?? '')}</span>
                          </div>
                              ...
                              <div className="bottom-0 w-[536px] cursor-pointer border-2 border-black bg-white shadow-brut-sm">
                                <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                                  {accounts.map((account) => (
                                    <a href="#" key={account.address} className="block w-full px-4 py-2 text-sm text-gray-700 hover:border-[#00B6A6] hover:bg-[#00B6A6] hover:text-gray-900" role="menuitem" onClick={() => popupHandler({ account })}>
                                      <div className='flex items-center'>
                                        <div className='mr-2 size-[25px] rounded-full bg-[#D9D9D9]' />
                                        <span className={`${FontManrope.className} text-base font-bold`}>{account?.meta.name}</span>
                                      </div>
                                      <div className='mt-1 flex items-center'>
                                        <TPLXWeb3Icon size={16} address={account.address} />
                                        <span className={`${FontManrope.className} ml-2 text-xs font-medium opacity-50`}>{getFirstFourLastFour(account.address)}</span>
                                      </div>
                                    </a>
                                  ))}
                                </div>
                      ...
                      <ConnectWallet onAccounts={setAccounts} />
                          ...
                          <div className='flex flex-col border-2 border-black bg-[#F8F8F8] px-2 py-1 shadow-brut-sm'>
                            <div className='flex items-center justify-between '>
                              <div className='flex items-center py-2'>
                                <span className={`${FontManrope.className} text-xl font-bold`}>Please Connect your wallet</span>
                              </div>
                              <ConnectWallet onAccounts={setAccounts} />
                            </div>
                          </div>
                          <div className='mt-4 flex items-center'>
                            <span className={`${FontManrope.className} ml-4 text-sm font-medium opacity-50`}>No Wallet Address</span>
                          </div>
                        </>
                      )}
                    </div>
                  </div>
                  <form onSubmit={handleSubmit(onSubmit)} className={`flex w-full flex-col self-center  px-5 py-2 ${getValues('hotkey') && "bg-[#00B6A6]"} bg-opacity-10`}>
                    <div className={`mb-4 mt-[20px] text-base font-bold opacity-80`}><span className={`rounded-3xl border-2 border-black px-2 py-1  text-sm uppercase opacity-100 shadow-brut-sm ${getValues('hotkey') ? "bg-[#00B6A6] text-white" : "bg-[#DADADA]"}`}>Step 2</span> VERIFY HOTKEY</div>
                    <div className={cn('mb-3')}>
                      <label htmlFor="hotkey" className={cn('block font-bold text-sm text-font-accent')}>HOTKEY<span
                        className="align-text-top text-red-500">*</span></label>
                      <InputField
                        className={'mt-3 '}
                        id="hotkey"
                        {...register('hotkey')}
                ...
                <>
                  <div className="my-2 flex flex-col justify-center">
                    <h1 className={`${FontSpaceMono.className} ml-5 text-xl font-bold uppercase`}>Application Successful!</h1>
                  </div>
                  <div className={`flex h-[215px] flex-col items-center border-t-2 border-black bg-[#DBF5E9]`}>
                    ...
                    <h1 className={`mb-4 text-2xl font-bold uppercase ${FontSpaceMono.className}`}>{`We've emailed your key`}</h1>
                    <p className={`w-[380px] text-center text-base font-semibold opacity-50 ${FontManrope.className}`}>{`We’ve sent it to ${getValues('email')} It should take up to five minutes to arrive`}</p>
                  </div>
