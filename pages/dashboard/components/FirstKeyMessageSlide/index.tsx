...
const FirstKeyMessageSlide = (props: Props ) => {
  ...
  return  <section className="sticky top-0 flex h-screen w-full justify-center overflow-hidden border-t-4 border-solid border-black">
    <Image
        src="/grid.svg"
        alt="Example Icon"
        width={100}
    ...
    className="flex size-full max-w-4xl flex-col items-center justify-evenly p-4">
      <div className="text-center">
        ...
        <motion.h2
          ...
          className={`${FontSpaceMono.className} text-4xl font-bold uppercase leading-tight md:text-5xl`}>Work anytime from anywhere</motion.h2>
        </div>
        <div className="overflow-hidden">
        <motion.p
         variants={elemAnimVariant}
         className="mt-4 text-lg leading-snug text-gray-500 md:text-xl">Enjoy the freedom to work on tasks at your convenience from any location</motion.p>
         </div>
      </div>
      {/* key visual graphic */}
            ...
            className="w-full rounded-b-2xl border-2 border-t-0 border-solid border-black bg-gray-100 p-4 pb-12">
              <div className="text-left">
                <motion.label 
                  ...
                  className={`${FontSpaceMono.className} font-bold`}>LINEAR SCALE <sup className="text-red-500">*</sup></motion.label>
                <motion.small 
                  variants={staggeredFadeInChildren}
                  className="block text-gray-500">Rate Bob’s sentiment from 1 (negative) to 5 (positive)</motion.small>
              </div>
              ...
              <div 
                ...
                <div className="relative h-[5px]">
                    ...
                    <motion.div 
                      ...
                      initial="hide"
                      animate={animState}
                      className="size-full origin-left bg-gray-300 "></motion.div>
                    {/* Steps */}
                    ...
                    {/* Knobs */}
                    <div className="absolute left-[-4px] top-1/2 size-[14px] -translate-y-1/2 cursor-pointer rounded-full border border-solid border-black bg-primary">
                    </div>
                </div>
              </div>
          </motion.div>
