...
const PayoutRecord = ({ taskName, status, date, amount } : Props) => {
  return (
    <div className="flex w-full gap-4 md:w-auto md:items-center">
      ...
      className="relative flex w-full flex-col gap-1 rounded-xl border-2 border-solid border-black bg-white px-4 py-2 shadow-brut-sm md:w-auto md:flex-row md:items-center md:gap-4 md:px-6 md:py-4">
        ...
        <div className="md:w-[80px]">
          <span className="font-semibold">
            <div className={`${FontSpaceMono.className} flex w-min items-center justify-center rounded-full bg-[#00B8A8] bg-opacity-20 px-3 py-1 text-sm text-[#00B8A8]`}>
              {status}
            </div>
          </span>
        </div>
        ...
        <div className="block w-[140px] md:hidden">
          <span className="text-md font-semibold text-[#00B8A8]">
      ...
      <motion.div
      variants={staggeredFadeInChildren}
      className="hidden h-min w-[140px] justify-center rounded-xl  border-2 border-solid border-black bg-[#00B8A8] bg-opacity-[0.15] py-2 shadow-brut-sm md:flex md:h-full md:py-4">
        <span className="block text-lg font-semibold text-[#00B8A8]">
          <CountUp start={0} end={amount} duration={2}  decimals={2} decimal="." prefix='+' suffix=' stTAO' enableScrollSpy />
        </span>
      </motion.div>
    </div>
