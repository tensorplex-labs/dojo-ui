...
export const SignIn: React.FC<Props> = ({ accounts, onCancel, onSignedIn }) => {
  ...
  const handleSignIn = async () => {
    try {
      ...
      const nonceRes = await fetch(`https://dojo-api.tensorplex.dev/api/v1/auth/${selectedAccount.address}`)
      ...
      const verifyRes = await fetch("https://dojo-api.tensorplex.dev/api/verify", {
        method: "POST",
  ...
  return (
    <div className="h-full flex flex-1 flex-col bg-black">
